...
def main(dir_data="../data",dir_plain="../plain",dir_clump="../clump",listfile="list.txt"):
    ...
    plt.loglog(r_scaled_array,sum_kplain[ind50]/sum_kdata[ind50],'g',label='$C(r)\equiv1$ and $\eta(r)\equiv1$')
    plt.fill_between(r_scaled_array,sum_kplain[ind16]/sum_kdata[ind50],sum_kplain[ind84]/sum_kdata[ind50],color='lightgreen',alpha=0.5)
    plt.loglog(r_scaled_array,sum_kclump[ind50]/sum_kdata[ind50],'k',label='$C(r)\equiv1$ and $\eta(r)$ set free')
    plt.fill_between(r_scaled_array,sum_kclump[ind16]/sum_kdata[ind50],sum_kclump[ind84]/sum_kdata[ind50],color='lightgrey',alpha=0.5)
